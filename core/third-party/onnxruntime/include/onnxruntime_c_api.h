...
typedef enum ONNXTensorElementDataType {
  ...
  ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT4E2M1,  // maps to a pair of packed float4
                                             // values (size == 1 byte)
...
typedef struct OrtAllocator {
   ...
   * these allocations.
   *
   * \return pointer to an allocated block of `size` bytes. nullptr if size was
   ...
   *
   * Return a pointer to the OrtKeyValuePairs structure that contains the
   * statistics of the allocator. The user should call
   * OrtApi::ReleaseKeyValuePairs when done.
   *
   ...
   * - NumReserves: Number of reserves. (Number of calls to Reserve() in
   * arena-based allocators)
   * - NumArenaExtensions: Number of arena extensions (Relevant only for arena
   * based allocators)
   * - NumArenaShrinkages: Number of arena shrinkages (Relevant only for arena
   * based allocators)
   * - MaxAllocSize: The max single allocation seen.
   *
   * The allocator is free to add other entries as appropriate.
   *
   * \note Implementation of this function is optional and GetStats may be set
   * to a nullptr. If the OrtAllocator is wrapping an internal ORT allocator
   * that does not implement GetStats the returned OrtKeyValuePairs instance
   * will be empty.
   *
   * \since 1.23
   */
   ...
   * \param[in] size Size of the allocation in bytes. nullptr if size was 0 or
   * allocation failed.
   * \param[in] stream The stream to allocate on.
   *
   * \return pointer to an allocated block of `size` bytes
   *
   * \note Implementation of this function is optional and AllocOnStream may be
   * set to a nullptr.
   * \since 1.23
   */
...
/** \brief These are the default EP selection policies used by ORT when doing
 ...
 *
 * This delegate is called by the EP selection code to allow the user to provide
 custom device selection logic.
 * The user can use this to select OrtEpDevice instances from the list of
 available devices.
 *
 ...
 * \param selected Pre-allocated array to populate with selected OrtEpDevice
 pointers from ep_devices.
 * \param max_selected The maximum number of devices that can be selected in the
 pre-allocated array. Currently the maximum is 8.
 * \param num_selected The number of selected devices.
 * \param state Opaque pointer. Required to use the delegate from other
 languages like C# and python.
 *
 * \return OrtStatus* Selection status. Return nullptr on success.
 *                    Use CreateStatus to provide error info. Use ORT_FAIL as
 the error code.
...
/** \brief Function called by ORT to write a buffer to a custom destination
 * (e.g., file, stream, etc.).
 *
 ...
 *                    Use CreateStatus to provide error info. Use ORT_FAIL as
 * the error code. ORT will release the OrtStatus* if not null.
 */
typedef OrtStatus *(ORT_API_CALL *OrtWriteBufferFunc)(
    _In_ void *state, _In_ const void *buffer, _In_ size_t buffer_num_bytes);

/** \brief Function called by ORT to allow user to specify how an initializer
 * should be saved, that is, either written to an external file or stored within
 * the model. ORT calls this function for every initializer when generating a
 * model.
 *
 * If the function implementation sets the `new_external_info` output parameter
 * to NULL, ORT stores the initializer data within the generated model.
 *
 ...
 * the initializer data in a file. In this case, ORT configures the model's
 * initializer to point to the location specified by the `new_external_info`
 * output parameter.
 *
 * \param[in] state Opaque pointer holding the user's state.
 * \param[in] initializer_name The initializer's name as a null-terminated
 * string.
 * \param[in] initializer_value OrtValue containing the initializer's data,
 * type, and shape.
 * \param[in] external_info If the initializer is originally stored in an
 * external file, `external_info` contains the file path, file offset, and the
 * data's byte size within the file. Otherwise, `external_info` is NULL if the
 * initializer is not originally stored in a file.
 * \param[out] new_external_info Output parameter set to a new
 * OrtExternalInitializerInfo instance indicating the location where the
 * function implementation stored the initializer data. The function
 * implementation must use `OrtApi::CreateExternalInitializerInfo()` to create
 * the instance. If the function implementation sets `new_external_info` to
 * NULL, ORT stores the initializers within the model.
 *
...
typedef struct OrtMIGraphXProviderOptions {
  ...
  int migraphx_fp16_enable;  // MIGraphX FP16 precision. Default 0 = false,
                             // nonzero = true
  int migraphx_fp8_enable;   // MIGraphX FP8 precision. Default 0 = false,
                            // nonzero = true
  ...
  /** \brief MIGraphX memory limit (To use all possible memory pass in maximum
   * size_t) Defaults to SIZE_MAX.
   *   \note If a ::OrtArenaCfg has been applied, it will override this field
   */
...
struct OrtApi {
   ...
   * ::OrtMemoryInfo
   *
   * The allocator wraps the internal allocator from the OrtSession and becomes
   * invalid when the session does.
   *
   * \param[in] session
   ...
   * maximum extend size if arena strategy is `kNextPowerOfTwo`. It is not an
   ...
   * \param[in] len Number of bytes stored in data for ORT_OP_ATTR_STRING.
                    Number of elements if data represents an array (e.g.,
   ORT_OP_ATTR_INTS). Otherwise, set to 1.
   ...
   * \param[in] type_constraint_names Name of the type constraints, such as "T"
   * or "T1"
   ...
   *
   * QNN supported keys:
   *   "backend_type": Type of QNN backend. Specifies a backend path that is the
   *associated QNN backend library file name. E.g., given backend type "htp", on
   *Windows, the backend path would be "QnnHtp.dll", and on other platforms, it
   *would be "libQnnHtp.so". Mutually exclusive with "backend_path". Available
   *options:
   *      -# "cpu"
   *      -# "gpu"
   *      -# "htp": Default.
   *      -# "saver"
   *      -# "ir"
   *   "backend_path": File path to QNN backend library. Mutually exclusive with
   *"backend_type". "profiling_level": QNN profiling level. Available options:
   *      -# "off": Default.
   *      -# "basic"
   *      -# "detailed"
   ...
   *   "dump_qnn_ir_dlc": Use the QnnIr backend library to write .dlc files for
   *each subgraph dispatched to QNN. When enabled, inference results will be
   *incorrect. Use only for debugging.
   *      -# "0": Default: disabled
   *      -# "1": enabled
   *   "dump_qnn_ir_dlc_dir": Set the directory into which QnnIr will be
   *configured to write QNN graphs as .dlc files. Default is current working
   *directory. "qnn_ir_backend_path": File path to the QnnIr backend library. If
   *"dump_qnn_ir_dlc" is enabled, use this path instead of looking for the Ir
   *backend in the standard location. "qnn_saver_path": File path to the QNN
   *Saver backend library. If specified, QNN Saver will be enabled and will dump
   *QNN API calls to disk for replay/debugging. QNN Saver produces incorrect
   *model inference results and may alter model/EP partitioning. Use only for
   *debugging. "qnn_context_priority": QNN context priority. Available options:
   *      -# "low"
   *      -# "normal": Default.
   *      -# "normal_high"
   *      -# "high"
   *   "htp_graph_finalization_optimization_mode": Set the optimization mode for
   *graph finalization on the HTP backend. Available options:
   *      -# "0": Default.
   ...
   *number. Refer to the QNN SDK documentation for valid values. Defaults to "0"
   *(unknown). "htp_arch": The minimum HTP architecture the driver will use to
   *select compatible QNN operators. Available options:
   *      -# "0": Default (none).
   *      -# "68"
   *      -# "69"
   *      -# "73"
   ...
   *EP).
   *      -# "0": Disabled. QNN EP will handle quantization and dequantization
   *of graph I/O.
   ...
   *     where op_type is the name of the operation, op_package_path is the path
   *to the op package shared library, interface is the symbol name to register
   *the op life cycle functions, and target is the backend type. For more
   *     details, refer to:
   ...
   *
   * Can be used in a custom operator to get the logging severity level of the
   ...
   * specific OrtMemoryInfo object. NOTE: callers are responsible to release
   ...
   *
   * \param[in] options instance that was passed to the session at creation
   * time.
   * \param[in] cancel setting this to true after model loading process was
   * initiated will attempt to cancel the loading process. If cancellation is
   * successful, CreateSession() CreateSessionFromArray() or any other session
   * creation API that take session options as an argument will return an
   * OrtStatus indicating that session loading was canceled at user request,
   *            error code ORT_MODEL_LOAD_CANCELED.
   *            The APIs above would not return any valid Session instance. This
   * is the best case effort and the result is not guaranteed. The session may
   * have already been created and initialized before the cancellation request
   * was issued.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  ORT_API2_STATUS(SessionOptionsSetLoadCancellationFlag,
                  _Inout_ OrtSessionOptions *options, _In_ bool cancel);

  /** \brief Get the Compile API instance.
   *
   * Get the Compile API instance to compile ONNX models. Execution providers
   * that support compilation fuse a subgraph into an EPContext node that wraps
   * a provider-specific binary representation of the subgraph. For more details
  ...
  /** \brief Get the value associated with a key in the OrtKeyValuePairs
   * instance.
   *
   * \param[in] kvps OrtKeyValuePairs instance.
   ...
   * \since Version 1.22.
   */
  const char *(ORT_API_CALL *GetKeyValue)(_In_ const OrtKeyValuePairs *kvps,
                                          _In_ const char *key);
   ...
   * \since Version 1.22.
   */
  void(ORT_API_CALL *RemoveKeyValuePair)(_In_ OrtKeyValuePairs *kvps,
                                         _In_ const char *key);
   ...
   *
   * The library must export 'CreateEpFactories' and 'ReleaseEpFactory'
   * functions. See OrtEpApi for more details.
   *
   * \param[in] env The OrtEnv instance to register the library in.
   * \param[in] registration_name The name to register the execution provider
   * library under.
   ...
   *
   * ORT will call ReleaseEpFactory for all factories created by the library,
   * and unload the library.
   *
   * You <b>MUST</b> ensure there are no Session instances using execution
   * providers created by the library before calling this function.
   *
   * \param[in] env The OrtEnv instance to unregister the library from.
   * \param[in] registration_name The name the execution provider library was
   * registered under.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * Each OrtEpDevice instance contains details of the execution provider and
   * the device it will use.
   *
   * \param[in] env The OrtEnv instance to query.
   * \param[out] ep_devices The OrtEpDevice instances that the execution
   * provider will use.
  ...
  /** \brief Append the execution provider that is responsible for the selected
   ...
   * \param[in] ep_devices One or more OrtEpDevice instances to create an
   * execution provider for. Obtain from GetEpDevices. All OrtEpDevice instances
   * must be from the same execution provider. It is only necessary to provide
   * multiple OrtEpDevices if you want to use the same execution provider for
   * multiple devices. e.g. the EP is capable of running on GPU and NPU.
   * \param[in] num_ep_devices Number of OrtEpDevice instances.
   * \param[in] ep_option_keys Optional keys to configure the execution
   * provider.
   * \param[in] ep_option_vals Optional values to configure the execution
   * provider.
   ...
   *
   * Allows users to specify a device selection policy for automatic execution
   * provider (EP) selection. If custom selection is required please use
  ...
  /** \brief Set the execution provider selection policy delegate for the
   * session.
   *
   * Allows users to provide a custom device selection policy for automatic
   * execution provider (EP) selection.
   *
   * \param[in] session_options The OrtSessionOptions instance.
   * \param[in] delegate Delegate callback for custom selection.
   * \param[in] delegate_state Optional state that will be passed to the
   * delegate callback. nullptr if not required.
   *
   * \since Version 1.22
   */
   ...
   *
   * \note This is not a unique identifier. It identifies the hardware type when
   * combined with vendor id.
   * \since Version 1.22.
   */
   ...
   *
   * \param[in] device The OrtHardwareDevice instance to query.
   * \return An OrtKeyValuePairs instance containing the metadata for the
   * device. Note: ORT owns the instance so the user must not call
   * ReleaseKeyValuePairs with it.
   *
   * \since Version 1.22.
   */
   ...
   *
   * \param[in] ep_device The OrtEpDevice instance to query.
   * \return An OrtKeyValuePairs instance containing the execution provider
  ...
  /** \brief Compute total size in bytes of the tensor data contained in an
   * OrtValue.
   *
   * Returns the total number of bytes used to store the tensor data. For
   * numeric tensors, this is sizeof(element_type) * total_element_count.
   * OrtValues that are not tensors or that are tensors that contain strings
   ...
   * \since Version 1.23
   */
  ORT_API2_STATUS(GetTensorSizeInBytes, _In_ const OrtValue *ort_value,
                  _Out_ size_t *size);

  /** \brief Calls OrtAllocator::GetStats function
   *
   * Return a pointer to the OrtKeyValuePairs structure that contains the
   * statistics of the allocator and the user should call
   ...
   * \param[out] out A pointer to the OrtKeyValuePairs instance that contains
   * the stats
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   * \param[in] vendor_id PCI Vendor ID. Use 0 for a generic allocator (e.g.
   * WebGPU).
   * \param[in] device_id Device ID if there are multiple devices of the same
   * type. e.g. 2 GPU devices.
   * \param[in] mem_type Memory type. Use OrtDeviceMemoryType_DEFAULT for device
   * memory, and OrtDeviceMemoryType_HOST_ACCESSIBLE (if applicable) for memory
   * used to transfer between the device and the CPU. Use the device_type and
   * device_id of the GPU/NPU that the memory is also accessible to.
   * \param[in] alignment Alignment of the memory if required. Pass 0 for
   * default alignment.
   * \param[in] allocator_type Allocator type. If
   * OrtAllocatorType::OrtArenaAllocator, the ORT arena will be used. Caveat:
   * Support for OrtArenaAllocator is currently limited to usage of internal ORT
   *                           allocators via
  ...
  ORT_API2_STATUS(CreateMemoryInfo_V2, _In_ const char *name,
                  ...
                  _In_ enum OrtDeviceMemoryType mem_type, _In_ size_t alignment,
  ...
  /** \brief Get the OrtNode that produces the value represented by the given
   * OrtValueInfo. Optionally returns the associated output index.
   *
   * \param[in] value_info The OrtValueInfo instance.
   * \param[out] producer_node Output parameter set to the OrtNode that produces
   * the OrtValueInfo.
   * \param[out] producer_output_index Optional output parameter set to the
   * OrtNode instance's output index that produces the value. Ignored if set to
   * NULL.
   * \snippet{doc} snippets.dox OrtStatus Return Value
   * \since Version 1.23.
   */
   ...
   *
   * Only nodes are considered "consumers" by this function. To check if an
   * OrtValueInfo is a graph output, call ValueInfo_IsGraphOutput().
   *
   * A single OrtNode may use a single value for more than one input (e.g.,
   * Mul(x, x)), so the returned `num_consumers` may be larger than the number
   * of unique OrtNode instances that consume the value.
   *
   * \param[in] value_info The OrtValueInfo instance.
   * \param[out] num_consumers Output parameter set to the number of consumers
   * of the value.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Returns information (OrtNode and input index) for all consumer
   * nodes that use the value as an input.
   *
   * Only nodes are considered "consumers" by this function.
   *
   * Caller provides 2 pre-allocated arrays that will be filled with the OrtNode
   * and input index values. Use ValueInfo_GetValueNumConsumers() to get the
   * number of consumers of the value.
   *
   * An OrtNode instance may appear multiple times if it uses the given value
   * more than once. Example: For a node MulNode(x, x) that consumes the value
   * 'x' twice, the following is returned:
   *   - nodes: [MulNode, MulNode]
   *   - input_indices: [0, 1]
   *
   * \param[in] value_info The OrtValueInfo instance.
   * \param[out] nodes Pre-allocated array of size `num_consumers` that is
   * filled with OrtNode instances.
   * \param[out] input_indices Pre-allocated array of `num_consumers` elements
   * that is filled with input indices. Index is set to -1 for an "implicit"
   * input to a consumer node that contains a subgraph (e.g., If, Loop) with
   * nodes that use the value internally.
   * \param[in] num_consumers The size of the `consumer_nodes` and
   * `consumer_input_indices` arrays. Typical usage sets this to the value of
   * ValueInfo_GetValueNumConsumers(). An error status is returned if
   * `num_consumers` is less than the number of actual consumers.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
  ...
  /** \brief Get the underlying initializer value, as an OrtValue, from the
   * given OrtValueInfo.
   *
   * Sets the output parameter to NULL if the given OrtValueInfo does not
   * represent an initializer. Does not return an error status in this case.
   *
   ...
   * initializers, ORT memory maps the initializer data on the first call to
   * this function. If caller needs custom memory mapping, use
   * ValueInfo_GetExternalInitializerInfo to get the location of the initializer
   * data.
   *
   * \param[in] value_info The OrtValueInfo instance.
   * \param[out] initializer_value Output parameter set to the initializer value
   * or NULL. Do not cache the OrtValue as it is released when the owning
   * OrtGraph is released.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
  ...
  /** \brief Get information about an external initializer (e.g., filepath, file
   * offset, byte size).
   *
   * Sets the output parameter `info` to NULL if the given OrtValueInfo does not
   * represent an initializer with external data. In this case, a NULL status
   * (non-error) is returned.
   *
   * \param[in] value_info The OrtValueInfo instance.
   * \param[out] info Output parameter set to an OrtExternalInitializerInfo
   * instance that can be used to query file path, file offset, etc. ORT sets
   * this to NULL if the OrtValueInfo does not represent an external
   * initializer. Must release with ReleaseExternalInitializerInfo.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   */
  ORT_API2_STATUS(ValueInfo_GetExternalInitializerInfo,
                  _In_ const OrtValueInfo *value_info,
                  _Outptr_result_maybenull_ OrtExternalInitializerInfo **info);

  /** \brief Returns a boolean indicating if the given value is a required graph
   * input.
   *
   * For ONNX IR version < 4, all graph inputs without a matching initializer
   * are required.
   *
   * For ONNX IR version >=4, a graph input with a matching initializer is an
   * optional graph input with the initializer serving as the default value.
   *
   * \param[in] value_info The OrtValueInfo instance representing the graph
   * value.
   * \param[out] is_required_graph_input Output parameter set to true if the
   * graph value is a required graph input.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
  ...
  /** \brief Returns a boolean indicating if the given value is an optional
   * graph input.
   *
   * Optional graph inputs were introduced in ONNX IR version 4. For ONNX IR
   * version >=4, a graph input with a matching initializer is an optional graph
   * input with the initializer serving as the default value. The matching
   * initializer is also known as a non-constant initializer.
   *
   * \param[in] value_info The OrtValueInfo instance representing the graph
   * value.
   * \param[out] is_optional_graph_input Output parameter set to true if the
   * graph value is an optional graph input.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   * \param[in] value_info The OrtValueInfo instance representing the graph
   * value.
   * \param[out] is_graph_output Output parameter set to true if the graph value
   * is a graph output.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Returns a boolean indicating if the given value is a constant
   * initializer.
   *
   * For ONNX IR version < 4, all initializers are constant.
   *
   * For ONNX IR version >=4, an initializer that serves as the default value
   * for a matching graph input is not a constant initializer.
   *
   * \param[in] value_info The OrtValueInfo instance representing the graph
   * value.
   * \param[out] is_constant_initializer Output parameter set to true if the
   * graph value is a constant initializer.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
  ...
  /** \brief Returns a boolean indicating if the given value is defined in an
   * outer scope.
   *
   * Certain operator types (e.g., If and Loop) contain nested subgraphs. This
   * function enables determining whether a value is defined in a parent node's
   * graph.
   *
   * \param[in] value_info The OrtValueInfo instance representing the graph
   * value.
   * \param[out] is_from_outer_scope Output parameter set to true if the value
   * is defined in an outer scope (i.e., a parent graph).
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
  ...
  /** \brief Get the filepath to the model from which an OrtGraph is
   * constructed.
   *
   * \note The model's filepath is empty if the filepath is unknown, such as
   * when the model is loaded from bytes via CreateSessionFromArray.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] model_path Output parameter set to the model's null-terminated
   * filepath. Set to an empty path string if unknown.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   *
   * \note An operator set is uniquely identified by the (domain, opset_version)
   * pair. All models must have at least one entry that specifies which entry of
   * the ONNX operator set is used. The ONNX domain is represented by an empty
   * string.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] num_operator_sets Output parameter set to the number of
   * operator sets that the graph's model uses.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] domains Pre-allocated array of `num_operator_sets` elements
   * that is filled with null-terminated domain names.
   * \param[out] opset_versions Pre-allocated array of `num_operator_sets`
   * elements that is filled with the opset version of the corresponding domain
   * in the `domains` array.
   * \param[in] num_operator_sets The size of the `domains` and `opset_versions`
   * arrays. Typical usage sets this to the result of
   * Graph_GetNumOperatorSets(). An error status is returned if
   ...
   *
   * \note The count includes initializers that are included in the list of
   * graph inputs.
   *
   * \param[in] graph The OrtGraph instance.
   ...
   *
   * \note The result includes initializers that are included in the list of
   * graph inputs.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] inputs Pre-allocated array of `num_inputs` elements that is
   * filled with the graph's inputs.
   * \param[in] num_inputs The size of the `inputs` array.
   *                       Typical usage sets this to the result of
   * Graph_GetNumInputs(). An error status is returned if `num_inputs` is less
  ...
  ORT_API2_STATUS(Graph_GetInputs, _In_ const OrtGraph *graph,
                  _Out_writes_(num_inputs) const OrtValueInfo **inputs,
                  _In_ size_t num_inputs);

  /** \brief Returns the number of graph outputs.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] num_outputs Output parameter set to the number of graph
   * outputs.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] outputs Pre-allocated array of `num_outputs` elements that is
   * filled with the graph's outputs.
   * \param[in] num_outputs The size of the `outputs` array.
   *                        Typical usage sets this to the result of
   * Graph_GetNumOutputs(). An error status is returned if `num_outputs` is less
   * than the number of graph outputs.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] num_initializers Output parameter set to the number of graph
   * initializers.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   * For ONNX IR version < 4, all initializers are constant.
   *
   * For ONNX IR version >= 4, an initializer with a name that matches a graph
   * input is considered a non-constant initializer.
   *
   * Call ValueInfo_GetInitializerValue to get the initializer's data.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] initializers Pre-allocated array of `num_outputs` elements that
   * is filled with the initializers.
   * \param[in] num_initializers The size of the `initializers` array. Typical
   * usage sets this to the result of Graph_GetNumInitializers(). An error
   * status is returned if `num_initializers` is less than the number of graph
   * initializers.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * The nodes are sorted using a stable topological ordering. Callers are
   * responsible for maintaining their own node ordering if a different order is
   * required.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] nodes Pre-allocated array of `num_nodes` elements that is
   * filled with the graph's nodes.
   * \param[in] num_nodes The size of the `nodes` array. Typical usage sets this
   * to the result of Graph_GetNumNodes(). An error status is returned if
   ...
   *
   * Certain operator types (e.g., If and Loop) contain nested subgraphs. This
   * function enables access to the parent node (e.g., the If and Loop node)
   * from a nested subgraph.
   *
   * \param[in] graph The OrtGraph instance.
   * \param[out] node Output parameter that is set to the graph's parent node.
   *                  Set to NULL if a parent node does not exist (e.g., for a
   * top-level graph).
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Returns an OrtGraph that contains a subset of nodes in the source
   * OrtGraph.
   *
   * \note The lifetime of "dst_graph" is tied to that of "src_graph", as they
   * both internally reference the same underlying graph.
   *
   * \param[in] src_graph The source OrtGraph instance.
   * \param[in] nodes A subset of the nodes/OrtNodes in 'graph'.
   * \param[in] num_nodes Number of nodes.
   * \param[out] dst_graph An OrtGraph created from a given set of nodes. Must
   * be released by calling ReleaseGraph.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   */
  ORT_API2_STATUS(Graph_GetGraphView, _In_ const OrtGraph *src_graph,
                  _In_ const OrtNode **nodes, _In_ size_t num_nodes,
   ...
   *
   * The node's identifier is only unique in the node's parent graph. Different
   * nested subgraphs (e.g., subgraphs contained by If and Loop nodes) may reuse
   * identifiers.
   *
   ...
   *
   * \param[in] node The OrtNode instance.
   * \param[out] operator_type Output parameter set to the name of the node's
   * operator type.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Get the opset version in which the given node's operator type was
   * first defined.
   *
   * \param[in] node The OrtNode instance.
   * \param[out] since_version The opset version in which the node's operator
   * type was first defined.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   * \param[in] node The OrtNode instance.
   * \param[out] inputs Pre-allocated array of `num_inputs` elements that is
   * filled with the node's inputs.
   * \param[in] num_inputs The size of the `inputs` array.
   *                       Typical usage sets this to the result of
   * Node_GetNumInputs(). An error status is returned if `num_inputs` is less
   ...
   * \param[in] node The OrtNode instance.
   * \param[out] outputs Pre-allocated array of `num_outputs` elements that is
   * filled with the node's outputs.
   * \param[in] num_outputs The size of the `outputs` array.
   *                        Typical usage sets this to the result of
   * Node_GetNumOutputs(). An error status is returned if `num_outputs` is less
   * than the number of node outputs.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   *
   * Certain operator types (e.g., If and Loop) contain nested subgraphs. The
   * internal nodes within the nested subgraphs may use values from the outer
   * scope. Those "outer scope" values are considered implicit inputs to the
   * node that contains the subgraphs (e.g., the If or Loop node).
   *
   * \param[in] node The OrtNode instance.
   * \param[out] num_implicit_inputs Output parameter set to the number of node
   * implicit inputs.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Get the implicit inputs, as OrtValueInfo instances, that are used
   * within the given node's subgraphs.
   *
   * \note Only certain operator types (e.g., If and Loop) contain nested
   * subgraphs. The internal nodes within the nested subgraphs may use values
   * from the outer scope. Those "outer scope" values are considered implicit
   * inputs to the node that contains the subgraphs (e.g., the If or Loop node).
   *
   * \param[in] node The OrtNode instance.
   * \param[out] implicit_inputs Pre-allocated array of `num_implicit_inputs`
   * elements that is filled the node's implicit inputs.
   * \param[in] num_implicit_inputs The size of the `implicit_inputs` array.
   * Typical usage sets this to the result of Node_GetNumImplicitInputs(). An
   * error status is returned if `num_implicit_inputs` is less than the number
   * of node implicit inputs.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   *
   * \param[in] node The OrtNode instance.
   * \param[out] num_attributes Output parameter set to the number of node
   * attributes.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * \param[in] node The OrtNode instance.
   * \param[out] attributes Pre-allocated array of `num_attributes` elements
   * that is filled with the node's attributes.
   * \param[in] num_attributes The size of the `num_attributes` array.
   *                           Typical usage sets this to the result of
   * Node_GetNumAttributes(). An error status is returned if `num_attributes` is
   * less than the number of node attributes.
   *
   * \note ONNX Runtime automatically sets optional (unset) attributes to their
   * default values if the default value is a constant expression that does not
   * depend on other tensor/model characteristics. Conv's 'kernel_shape'
   * attribute is an example of an optional attribute that does not have a
   * constant default value. This function does not provide any unset optional
   ...
   * \param[out] attribute Output parameter set to the OrtOpAttr instance if an
   * attribute by the given name exists. For an unset optional attribute,
   * `attribute` is set to NULL and a non-error status is returned. For an
   * invalid attribute name, `attribute` is set to NULL and an error status with
   ...
   * \param[out] attr_tensor If successful, contains the 'TENSOR' attribute as a
   ...
   * \since Version 1.23.
   */
  ORT_API2_STATUS(OpAttr_GetTensorAttributeAsOrtValue,
   ...
   *
   * \param[in] attribute The OrtOpAttr instance.
   * \param[out] name Output parameter set to the attribute's name. The name is
   ...
   *
   * \param[in] node The OrtNode instance.
   * \param[out] num_subgraphs Output parameter set to the number of node
   * subgraphs.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Get the subgraphs, as OrtGraph instances, contained by the given
   * node.
   *
   * \note Only certain operator types (e.g., If and Loop) contain nested
   * subgraphs. ONNX nodes store subgraphs in their attributes, however, this
   * function must be used to obtain subgraphs from an OrtNode.
   *
   * \param[in] node The OrtNode instance.
   * \param[out] subgraphs Pre-allocated array of `num_subgraphs` elements that
   * is filled with the node's subgraphs.
   * \param[in] num_subgraphs The size of the `num_subgraphs` array.
   *                          Typical usage sets this to the result of
   * Node_GetNumSubgraphs(). An error status is returned if `num_subgraphs` is
   * less than the number of node subgraphs.
   * \param[out] attribute_names Optional pre-allocated array of `num_subgraphs`
   * elements that is filled with the attribute names that correspond to the
   * subgraphs. Ignored if set to NULL.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
   ...
   *
   * \param[in] node The OrtNode instance.
   * \param[out] graph Output parameter set to the node's OrtGraph. Can be set
   * to NULL if the node is not currently contained by a graph.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23.
  ...
  /** \brief Returns the execution provider name that this node is assigned to
   * run on. Returns NULL if the node has not been assigned to any execution
   * provider yet. For plugin execution providers, the name is the one returned
   * by OrtEp::GetName.
   *
   * \param[in] node The OrtNode instance.
   * \param[out] out Output execution provider type and can be NULL if node has
   * not been assigned.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Get the relative path to the file that stores the initializer's
   * data.
   *
   * \note The path is relative to the filesystem directory where the ONNX model
   * was stored. Caller can use Graph_GetModelPath to get the model's full path
   * and construct the absolute path to the external initializer file if
   * necessary.
   *
   * \param[in] info The OrtExternalInitializerInfo instance.
   * \return The relative path to the file that stores the initializer's data.
  ...
  /** \brief Get the byte offset within the file where the initializer's data is
   * stored.
   *
   * \param[in] info The OrtExternalInitializerInfo instance.
   * \return The byte offset where the initializer's data is stored within the
   * file.
   *
   * \since Version 1.23.
   */
   ...
   *
   * If a run configuration entry with key `config_key` doesn't exist,
   * `config_value` will be set to NULL.
   *
   * `config_key`s are defined in onnxruntime_run_options_config_keys.h.
   *
   * \param[in] options The OrtRunOptions instance.
   * \param[in] config_key The configuration entry key. A null-terminated
   * string.
   * \return The configuration entry value. Either a null-terminated string if
   * the entry was found. nullptr otherwise.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23
   ...
   * \return A pointer to the OrtMemoryInfo for the device. This may be nullptr
   * if not set. If memory_type is OrtDeviceMemoryType_DEFAULT and nullptr is
   * returned the EP uses CPU memory.
   *
   * \since Version 1.23
   */
  ...
  /** \brief Create/replace a shared allocator for the OrtEpDevice in the
   * OrtEnv.
   *
   * OrtEpDevice maps to the EP factory, and the factory provides the allocator
   * implementation.
   *
   * Both OrtDeviceMemoryType_DEFAULT and OrtDeviceMemoryType_HOST_ACCESSIBLE
   * are optional for an EP to provide. It is EP implementation dependent as to
   * what is available.
   *
   * If a shared allocator already exists for the OrtEpDevice and
   * OrtDeviceMemoryType, it is replaced. This allows changing the shared
   * allocator configuration from the default. e.g. adding an arena.
   *
   * \param[in] env The OrtEnv instance to create the shared allocator in.
   * \param[in] ep_device The OrtEpDevice instance to create the shared
   * allocator for.
   * \param[in] mem_type The memory type to use for the shared allocator.
   * \param[in] allocator_type The type of allocator to create. Only
   * OrtDeviceAllocator is valid currently.
   * \param[in] allocator_options Optional key-value pairs to configure the
   * allocator. If arena based, see
   *                              include/onnxruntime/core/framework/allocator.h
   * for the keys and values that can be used.
   * \param[out] allocator A pointer to the created shared allocator. Owned by
   * the OrtEnv instance.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23
   */
  ORT_API2_STATUS(CreateSharedAllocator, _In_ OrtEnv *env,
                  _In_ const OrtEpDevice *ep_device,
   ...
   *
   * By default there is a shared allocator created for all OrtEpDevice
   * instances, so if you get the OrtMemoryInfo from the OrtEpDevice using
   * EpDevice_MemoryInfo a shared allocator is guaranteed to exist.
   *
   ...
   * \param[in] mem_info The OrtMemoryInfo instance to get the shared allocator
   * for.
   * \param[out] allocator A pointer to the shared allocator, or nullptr if no
   * shared allocator exists for the given memory info.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23
   */
  ORT_API2_STATUS(GetSharedAllocator, _In_ OrtEnv *env,
                  _In_ const OrtMemoryInfo *mem_info,
                  _Outptr_result_maybenull_ OrtAllocator **allocator);

  /** \brief Release a shared allocator from the OrtEnv for the OrtEpDevice and
   * memory type.
   *
   * This will release the shared allocator for the given OrtEpDevice and memory
   * type. If no shared allocator exists, this is a no-op.
   *
   * \param[in] env The OrtEnv instance to release the shared allocator from.
   * \param[in] ep_device The OrtEpDevice instance to release the shared
   * allocator for.
   ...
   *
   * The memory info can be used to determine where the input tensors are
   * required.
   *
   * The session must be fully initialized before calling this function as the
   * input locations are not known until this has occurred.
   *
   * \param[in] session The OrtSession instance.
   * \param[out] inputs_memory_info Pre-allocated array of size `num_inputs`
   * that will be filled with the OrtMemoryInfo* value for each input. The order
   * is the same as returned by SessionGetInputName.
   * \param[in] num_inputs The number of inputs in the session. Must match
   ...
   *
   * The memory info can be used to determine the device the output tensors are
   * produced on. The user can pre-allocate an OrtValue using this information
   * or use IOBinding to keep the data on the device. ORT will copy the output
   * to CPU otherwise.
   *
   * The session must be fully initialized before calling this function as the
   * output locations are not known until this has occurred.
   *
   * \param[in] session The OrtSession instance.
   * \param[out] outputs_memory_info Pre-allocated array of size `num_outputs`
   * that will be filled with OrtMemoryInfo* values for each output. The order
   * is the same as returned by SessionGetOutputName.
   ...
   * SessionOptionsSetEpSelectionPolicy or
   * SessionOptionsSetEpSelectionPolicyDelegate, or if the OrtEpDevice was
   * manually added to the session using
   ...
   * CreateSyncStreamForEpDevice to asynchronously provide input to the
   * inference session Run.
   *
   * The session must be fully initialized before calling this function as the
   * assigned EPs are not known until this has occurred.
   *
   * \param[in] session The OrtSession instance.
   * \param[out] inputs_ep_devices Pre-allocated array of size `num_inputs` that
   * will be filled with OrtEpDevice* values for each input. The order is the
   * same as returned by SessionGetInputName.
   * \param[in] num_inputs The number of inputs in the session. Must match
   * SessionGetInputCount.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   ...
   *
   * An error code of ORT_NOT_IMPLEMENTED will be returned if the EP does not
   * support OrtSyncStream.
   *
   * \param[in] ep_device The OrtEpDevice instance to create the sync stream
   * for.
   * \param[in] stream_options Options for OrtSyncStream creation. May be
   * nullptr.
   * \param[out] stream Output parameter set to the created OrtSyncStream
   * instance.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * This returns the native handle for the stream. e.g. cudaStream_t for CUDA
   * streams.
   *
   ...
   *
   * The overall copy must be between a single source device and a single
   * destination device. i.e.
   ...
   *   - Use GetSharedAllocator to get the shared allocator for the
   * OrtMemoryInfo if you need to allocate memory on the device.
   *   - Use CreateTensorAsOrtValue, CreateTensorWithDataAsOrtValue or
   * CreateTensorWithDataAndDeleterAsOrtValue to create an OrtValue containing a
   * tensor depending on whether you have existing data or not, and whether you
   * want ORT to free the existing data once it is done with the OrtValue.
   *
   * \param[in] env The OrtEnv instance to use. The data transfer implementation
   * is provided by an execution provider that is registered in this OrtEnv.
   * \param[in] src_tensors Array of OrtValue instances containing the source
   * tensors to copy.
   * \param[in] dst_tensors Array of OrtValue instances to copy the source
   * tensors to.
   * \param[in] stream Optional OrtSyncStream that can be used to perform the
   * copy asynchronously. May be nullptr.
   * \param[in] num_tensors The number of tensors to copy. The size of
   * `src_tensors` and `dst_tensors` must match.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.23
  ...
  /** \brief Validate a compiled model's compatibility information for one or
   * more EP devices.
   *
   * \param[in] ep_devices The EP devices to validate against (e.g., from
   * GetEpDevices). All devices must belong to the same execution provider.
   * \param[in] num_ep_devices The number of EP devices provided.
   * \param[in] compatibility_info The compatibility info string produced when
   * the model was compiled.
   * \param[out] out_status The resulting compatibility status for the EP
   * devices.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * \param[in] filepath The relative path to the file that stores the
   * initializer's data. ORT copies this path string.
   * \param[in] file_offset The byte offset where the initializer's data is
   * stored within the file.
   * \param[in] byte_size The size in bytes of the initializer's data within the
   * file.
   * \param[out] out Output parameter set to the new OrtExternalInitializerInfo
...
struct OrtCustomOp {
  ...
  // types, this function can be overridden to return the specific memory types.
...
struct OrtModelEditorApi {
   ...
   * must be able to discover that opset version. Returns an error if the domain
...
typedef enum OrtCompileApiFlags {
  ...
  // Otherwise, a model with basic optimizations (ORT_ENABLE_BASIC) is still
  // generated by default.
  OrtCompileApiFlags_ERROR_IF_NO_NODES_COMPILED = 1 << 0,

  // Force compilation to return an error (ORT_FAIL) if a file with the same
  // filename as the output model exists.
  // Otherwise, compilation will automatically overwrite the output file if it
  // exists.
 ...
 *
 * Execution providers that support compilation fuse a subgraph into an
 * EPContext node that wraps a provider-specific binary representation of the
 * subgraph. For more details about the EPContext design, refer to:
 *  \htmlonly
 *  <a
 * href="https://onnxruntime.ai/docs/execution-providers/EP-Context-Design.html">EPContext
 * design document.</a>
 ...
 *
 *   status = compile_api->CreateModelCompilationOptionsFromSessionOptions(env,
 ...
 * ORT_TSTR("model.compiled.onnx")); status = compile_api->CompileModel(env,
 * compile_options);
...
struct OrtCompileApi {
  ...
  /** \brief Creates an OrtModelCompilationOptions object from an existing
   * OrtSessionOptions object.
   *
   * An OrtModelCompilationOptions object contains the settings used to generate
   * a compiled ONNX model. The OrtSessionOptions object has the execution
   * providers with which the model will be compiled.
   *
   ...
   * ModelCompilationOptions_SetGraphOptimizationLevel to enable graph
   * optimizations.
   *
   * \param[in] env OrtEnv object.
   * \param[in] session_options The OrtSessionOptions instance from which to
   * create the OrtModelCompilationOptions.
   ...
   *
   * The input model's location (e.g., file path or memory buffer) must be set
   * with either ModelCompilationOptions_SetInputModelPath or
   * ModelCompilationOptions_SetInputModelFromBuffer.
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] input_model_path Null terminated string of the path (wchar on
   * Windows, char otherwise).
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.22.
  ...
  /** \brief Sets the buffer that stores the bytes of the loaded ONNX model to
   * compile.
   *
   * The input model's location (e.g., file path or memory buffer) must be set
   * with either ModelCompilationOptions_SetInputModelPath or
  ...
  /** \brief Sets the file path for the output ONNX model generated by
   * CompileModel.
   *
   * The output model's location (e.g., file path or memory buffer) can be set
   * with either ModelCompilationOptions_SetOutputModelPath or
   * ModelCompilationOptions_SetOutputModelBuffer.
   *
   * If the output model's location is not set, ONNX Runtime will generate an
   * output file with a path based on the input model's file path. Examples:
   *   /Path/my_model.onnx -> /Path/my_model_ctx.onnx
   *   /Path/my_model -> /Path/my_model_ctx.onnx
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] output_model_path Null terminated string of the path (wchar on
   * Windows, char otherwise).
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Optionally sets the file that should store external initializers
   * for the compiled ONNX model. If not set, initializers are stored within the
   * model.
   *
   * Only initializers for nodes that were not compiled are stored in the
   * external initializers file. Compiled nodes contain their initializer data
   * within the `ep_cache_context` attribute of EPContext nodes. Refer to
   * ModelCompilationOptions_SetEpContextEmbedMode.
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] external_initializers_file_path Null terminated string of the
   * path to the file.
   * \param[in] external_initializers_size_threshold Initializers larger than
   * this threshold are stored in the file.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.22.
  ...
  /** \brief Configures model compilation to store the output compiled ONNX
   * model in a buffer.
   *
   * The caller passes an OrtAllocator that ONNX Runtime uses to allocate memory
   * for the buffer.
   *
   * The output model's location (e.g., file path or memory buffer) can be set
   * with either ModelCompilationOptions_SetOutputModelPath or
   ...
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] allocator The allocator used to allocate the buffer for the
   * compiled model.
   * \param[out] output_model_buffer_ptr Pointer to the buffer that stores the
   * compiled model.
   * \param[out] output_model_buffer_size_ptr Pointer set to the size of output
   * model in bytes.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   ...
   *
   * If enabled, the `ep_cache_context` attribute of EPContext nodes will store
   * the context binary data, which may include weights for compiled subgraphs.
   *
   * If disabled, the `ep_cache_context` attribute of EPContext nodes will
   * contain the path to the file containing the context binary data. The path
   * is set by the execution provider creating the EPContext node.
   *
   ...
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] embed_ep_context_in_model True to embed EPContext binary data
   * into the EPContext node `ep_cache_context` attributes.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   * \since Version 1.22.
   ...
   *
   * \param[in] env OrtEnv object.
   * \param[in] model_options The compilation options that defines compilation
   * options for a model.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Sets flags from OrtCompileApiFlags that represent one or more
   * boolean options to enable.
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   ...
   *
   * EP uses this information to decide the location and context binary file
   * name. Used while compiling model with input and output in memory buffer
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] output_directory Null terminated string of the path (wchar on
   * Windows, char otherwise).
   * \param[in] model_name Null terminated string of the model name (wchar on
   * Windows, char otherwise).
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
  ...
  /** \brief Sets a OrtWriteBufferFunc function that is called by ORT to write
   * out the output model's serialized ONNX bytes.
   *
   * The provided write function may be called repeatedly until then entire
   * output model has been written out. Each call to the write function is
   * expected to consume the entire input buffer.
   *
   * The output model's destination (e.g., file path, memory buffer, or stream)
   * can be set with any of the functions that begin with
   * ModelCompilationOptions_SetOutputModel____.
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] write_func The OrtWriteBufferFunc function called by ORT when
  ...
  /** \brief Sets a OrtGetInitializerLocationFunc function that is called by ORT
   * for every initializer in the generated model. Allows implementer to specify
   * whether initializers should be stored within the model or externally.
   *
   * \param[in] model_compile_options The OrtModelCompilationOptions instance.
   * \param[in] get_initializer_location_func The OrtGetInitializerLocationFunc
   * function called by ORT when to determine the location of the initializer.
   * \param[in] state Opaque state passed as the first argument to
